...
**Run integrity comes first.** A scorecard is only as trustworthy as its input, so before scoring the engine verifies the run file end to end and refuses on any violation: recorded `run_failure` records (identity/collision) make a run unscoreable; every game, request, and slate hash is recomputed from the embedded bundles; every decision's selection, line, and entry price is re-verified against the hash-verified bundle; and every model decision must be backed by a `valid` arm response for the same participant, game, and request hash — exactly three per response, none fabricated.
