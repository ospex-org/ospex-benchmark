...
- The extracted answer text (`answerText`), the **complete sanitized provider response body** it came from (`responseEnvelope`, with a sha256 over exactly the stored bytes; retained on a 2xx whatever the body turns out to be — sealed before any adapter reads it, so bytes that are not JSON and JSON in a shape the extractor cannot walk both keep it — a non-2xx error body keeping truncated detail instead), parsed output, repair request/response, provider response ID, response-reported model, timestamps, tokens, cost, and latency are retained, along with the per-attempt web-search audit: every executed query, every result reference, the billable search count, and any reason the audit is known to be partial. The envelope is what makes that audit re-derivable offline (`yarn replay:search-audit`): the audit is one parser's reading of the response, so a shape the parser did not recognize leaves an empty audit that only the retained body can tell apart from a model that never searched. The scheduled campaign path writes no run file at all — its durable record is a fire artifact — and it retains the same complete body per sent attempt, initial and repair alike, digest-bound into that artifact's `armDigest`. **`yarn replay:search-audit` reads run files only, and refuses a fire artifact by name rather than reporting a verdict on it** — a fire artifact is a single JSON object, not a record stream, and before that refusal existed the command read one as an empty archive and reported "envelopes unavailable" at exit 0. A fire artifact's envelopes are verified by `yarn verify:sidecar`, which recomputes their digests; re-running a *newer* extractor over an *older* fire artifact's retained bodies is the one capability not yet wired, and the bytes it needs are retained and digest-bound today.
