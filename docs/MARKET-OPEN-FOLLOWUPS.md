...
- **Activation:** separately approved source acquisition and service wiring, one authoritative local POSIX evidence root, credential isolation, deployment/recovery checks and operational relaunch. The benchmark writer now reclaims only provably dead local owners ([protocol](LOCAL-PROCESS-LOCK.md)); reader adoption and liveness alerts remain separate work. There is still no age-based lock stealing or unknown-spend override. Campaign/#122/#123 remain set aside, not deleted.
