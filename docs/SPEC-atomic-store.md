...
4. **`cohortSpendCapUsdMicros` is enforced with a conservative fixed administrative reservation** (`spendReservationPolicyVersion`/`providerAttemptReservationUsdMicros`); an unknown spend-reservation policy or a mismatched per-attempt amount **fails boot**.
...
fires                               -- one row per dispatch: idempotency + settle anchor
  ...
  spend_reserved_usd_micros bigint  -- this fire's reserved spend (caller-derived fixed reservation, recorded)
...
The hard guarantees are the **call count**, the **token/output bounds**, and this **conservative fixed reservation** — not exact external billing.
...
begin
  ...
  -- 4. store-derived call/slot magnitudes; spend = caller-derived fixed reservation for the EXACT retained scope (§4.5)
...
18. **Retained-scope digest binding:** the store selects the EXACT retained subset from the scope table (§4.5) and returns its `preparedBytesDigest`, so the dispatched bytes are bound to the retained scope; under `fixed-attempt-v1` the reserved spend is the **same fixed amount** for every nonempty subset (`spendΔ = roster × (1 + maxRepairs) × providerAttemptReservationUsdMicros`), so a smaller retained subset never changes it; a retained subset absent from the table refuses `scope_reservation_missing`.
19. **Fixed per-attempt reservation:** the per-fire spend reservation is `callΔ × providerAttemptReservationUsdMicros` with `callΔ = roster × (1 + maxRepairs)` — the same fixed amount per attempt (initial and repair alike), computed with checked `bigint` arithmetic; an unknown `spendReservationPolicyVersion` or a manifest amount that disagrees with the code-owned policy value fails boot/admission (§6).
