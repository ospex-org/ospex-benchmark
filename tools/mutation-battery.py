...
MUTANTS = [
    ...
    ('publisher reveals even when the seal was refused', 'src/servingPublisher.ts',
     ...
     "  tally.record(await settle(() => port.sealDecision(seal), budget.slice()), `seal ${what}`, log);"),
    ...
    ('a write may run past the deadline it started before', 'src/servingPublisher.ts',
     "    return Math.max(1, Math.min(this.perWrite, this.expiresAt - this.nowMs()));",
     "    return this.perWrite;"),
    ('the budget is not checked between a seal and its reveal', 'src/servingPublisher.ts',
    ...
    ('an unanswerable readiness question counts as ready', 'src/benchmarkServingClient.ts',
