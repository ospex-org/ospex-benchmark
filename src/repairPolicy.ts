 ...
 *
 * `CODE_MAX_REPAIRS_PER_ARM` DESCRIBES the runner's capability (the runner sends
 * exactly this many repairs) — it does not control the flow, and it is not a caller
 * knob. A behavior-correspondence test observes the repair-exhaustion path and
 * compares the actual repair count to it. Version-only, no digest: a future repair
 * token/spend bound is pinned by the separate token-bound policy digest, not here.
...
/** The runner's audited maximum repair attempts per arm (it sends exactly this many). */
