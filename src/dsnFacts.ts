...
/**
 * What node-postgres will do with a connection string — asked of the driver's
 * own parser rather than predicted.
 *
 * Two facts, and they are the only two anyone here has needed: the host it will
 * dial, and whether the string states an SSL policy of its own. Both matter for
 * the same reason. A bare `new Pool({ connectionString })` with no `ssl` option
 * and no SSL parameter in the URL sends the password and every row across the
 * network in cleartext, successfully and with no warning; and an `ssl` option
 * supplied beside a URL that states its own policy is silently replaced.
 *
 * ── WHY THIS ASKS THE DRIVER INSTEAD OF READING THE URL ──────────────────────
 *
 * Two callers each shipped a model of the driver, and both were wrong in ways
 * that put a credential on the wire in the clear. The campaign store searched
 * the text after the userinfo for `sslmode=` and read the host out of the
 * authority with a substring; the serving resolver parsed with `new URL()` and
 * inspected `sslmode` and `ssl` through `URLSearchParams`. Measured against the
 * pinned driver, between them:
 *
 ...
 *   postgres://u:pw@/db?sslmode=disable   userinfo with an empty host THROWS in
 *                                         `new URL()`; the driver's parser has a
 *                                         dummy-host fallback and applies the
 *                                         sslmode, so TLS was reported and
 *                                         plaintext was sent
 *   ?sslnegotiation=direct                not in the inspected parameter list,
 *                                         but it replaces a supplied ssl object,
 *                                         discarding the configured CA
 *   ?sslrootcert=... ?sslcert=... ?sslkey=...
 *                                         same, and each also reads a file
 ...
 *
 * So nothing here predicts what the driver will do. It calls the same
 * `pg-connection-string` module `pg` itself calls, kept on the same version
 * range so a single copy is hoisted, and reads the answer back:
 *
 *   the SSL policy    `ConnectionParameters` merges the parsed connection string
 *                     OVER the supplied config, so an own `ssl` key in the parse
 *                     result IS "the URL overrides whatever I attach". Nothing
 *                     else has to be enumerated — which is what retires the
 *                     `sslnegotiation` / `sslrootcert` / `sslcert` / `sslkey`
 *                     class permanently instead of extending a list that will
 *                     fall behind the driver again.
 *   the host          the parse result's host, which already has any `?host=`
 *                     override applied.
 *
 * Verified by differential test against a real `pg.Client` and `pg.Pool` over a
 * generated matrix of connection strings, in `store/connection.test.ts` and
 * `benchmarkServingConfig.test.ts`. The runs that need a server — a live
 * PostgreSQL with TLS off, and a listener reading the first bytes off the
 * socket — are recorded in the pull requests, since they need a non-loopback
 * address and cannot run in CI.
 *
 * ── WHY THERE IS NO RULE ABOUT DUPLICATE OR EMPTY SSL PARAMETERS ─────────────
 *
 * Because a second parser to detect them would reintroduce the defect above.
 * Every duplicate is resolved by the driver's own parser, so the answer here is
 * the answer the connection will have — `?sslmode=require&sslmode=` is
 * `unstated`, `?ssl=1&ssl=` is `plaintext`, and neither is ambiguous once you
 * stop guessing. What a caller does about `plaintext` is the caller's policy;
 * what it must not do is treat it as `unstated` and report TLS it will not get.
 *
 ...
 *   "encrypt but do not verify" is `no-verify`.
 */
...
/** Everything a caller needs, from ONE parse of the string. */
export interface DsnFacts {
 ...
 * is a driver error on connect, naming the file, not a silent plaintext send.
 *
 ...
 */
export function dsnFacts(databaseUrl: string): DsnFacts {
