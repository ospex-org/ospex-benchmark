...
export interface AdmissionAttemptLifecycle extends AttemptLifecyclePort {
   ...
   * Every lease is attempted even if one fails, and the complete failure set is reported.
   */
...
export function createAttemptLifecycle(dispatch: AuthorizedDispatch): AdmissionAttemptLifecycle {
  ...
  /** One release through the admission's own authority; only `released` is success. */
  const release = async (leaseId: string, what: string): Promise<void> => {
    let outcome: string;
    try {
      outcome = (await authority.releaseLease(leaseId)).outcome;
    } catch {
      // Never format the thrown value — a hostile raise must not escape as a raw error.
      throw new LifecycleFaultError(`${what} release threw`);
    }
    if (outcome !== 'released') throw new LifecycleFaultError(`${what} release was refused (${outcome})`);
  };
  ...
  return {
    ...
    async acquireRepair(armIndex: number, repairOrdinal: number): Promise<{ authorized: boolean }> {
      ...
      if (problem !== null) {
        ...
        try {
          const r = await authority.releaseLease(lease.leaseId);
          if (r.outcome !== 'released') cleanup = r.outcome;
        } catch {
          cleanup = 'threw';
        }
