...
function projectArmAttempts(
  ...
  const participant: ParticipantFacts = {
    participantId: entry.participantId,
    kind: entry.kind,
    labId: entry.labId,
    displayName: entry.displayName,
  };
  const roster = rosterFacts(entry, header);
...
function rosterFacts(entry: ProjectionParticipant, header: RunHeader): RosterFacts {
  return {
    armId: armIdFor(entry, header.baselinePolicyVersion),
...
function projectModelDecision(
  ...
  const seal: DecisionSeal = {
    ...
    roster: rosterFacts(entry, header),
    gameId,
